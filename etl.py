...
def process_song_file(cur, filepath):
    """
    Create song table and artist table from JSON file
    """
    # open song file
...
def process_log_file(cur, filepath):
    """
    - Create log table from JSON file
    - Create songplay table from the log table
    """
    # open log file
    df = pd.read_json(filepath, lines=True)
    ...
    for index, row in df.iterrows():
        ...
        songplay_data = (songid, artistid, row.userId, row.ts, row.userId, row.level, row.sessionId, row.location, row.userAgent)
        cur.execute(songplay_table_insert, songplay_data)


def process_data(cur, conn, filepath, func):
    """
    Processing all JSON file into postgre
    """
